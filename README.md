...
- ARK 文本模型：配置 `ARK_API_KEY`、`ARK_OPENAI_BASE_URL`、`ARK_MODEL` 后启用。
- ARK embedding：配置 `ARK_EMBEDDING_MODEL` 后 RAG 可切换远程 embedding。
- YOLO/CLIP：按需安装 `ultralytics`、`transformers`、`torch`，并设置 `YOLO_MODEL_PATH` 或 `VISION_MODEL=clip`。这些不是默认依赖。
- LangSmith：配置密钥和 tracing 开关后启用；未配置或失败不影响主业务。

## 答辩展示流程

1. 打开首页，展示倒计时、新闻、比赛和 AI 入口。
2. 在 AIStudio 输入“法国夺冠概率”，检查 Prediction Agent、本地概率和 ARK 解释。
3. 输入“法国队战术特点”和“姆巴佩近期状态”，展示 Team/Player Agent。
4. 创建球迷画像，刷新后读取，并生成日报。
5. 打开世界杯中心，展示 104 场赛程、12 组积分榜和淘汰赛图。
6. 打开球队、球员详情页。
7. 在新闻中心做可信度分析，并尝试联网刷新。
8. 上传正常图片和非法文件，展示视觉分析与错误提示。
9. 输入“法国夺冠”生成海报和文案，打开返回的海报链接。

## 常见错误

- `pnpm install` 解析 registry 失败：检查 DNS、代理或 registry 是否可访问。
- 开着代理但 pnpm 仍 DNS 失败：给当前 PowerShell 设置 `HTTP_PROXY`、`HTTPS_PROXY`，并保留 `NO_PROXY=127.0.0.1,localhost,::1`。
- ARK 返回 `local`：说明密钥未配置、超时、限流、鉴权失败或接口不可达，查看 `metadata.fallback_reason`。
- PowerShell 中文乱码：使用 UTF-8 代码页和 `PYTHONIOENCODING=utf-8`。
- SQLite 测试库无法删除：确认没有其他后端进程正在使用 `storage/test_worldcup_ai.db`。
- PostgreSQL 连接失败：安装匹配的 SQLAlchemy 驱动，并使用正确的 `DATABASE_URL`。
